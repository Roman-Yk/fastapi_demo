...
class ForeignKeyValidator:
    ...
    async def validate_single_reference(
        ...
        """
        try:
            await is_record_exists(self.db, model_class, record_id)
        except Exception as e:
            field_info = f" for field '{field_name}'" if field_name else ""
            raise HTTPException(
                status_code=400,
                detail=f"Invalid {model_class.__name__.lower()}_id{field_info}: {str(e)}"
            )
    
    async def validate_references_from_mapping(
        self, 
        data: Dict[str, Any], 
        validation_mapping: Dict[str, Type[DeclarativeBase]]
    ) -> None:
        """
        ...
        for field_name, model_class in validation_mapping.items():
            if field_name in data and data[field_name] is not None:
                task = self.validate_single_reference(model_class, data[field_name], field_name)
        ...
        if validation_tasks:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Foreign key validation failed: {str(e)}"
                )