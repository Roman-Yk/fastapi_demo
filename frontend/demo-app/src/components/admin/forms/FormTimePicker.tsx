...
interface FormTimePickerProps<T extends Record<string, any>, K extends keyof T> {
  label: string;
  source: K;
  formData: T;
...
export function FormTimePicker<T extends Record<string, any>, K extends keyof T>({
  label,
  source,
  formData,
  updateField,
  ...
  const handleAccept = () => {
    if (tempValue) {
      ...
      updateField(source, timeString as T[K], transform);
    }
    setOpened(false);
  };
